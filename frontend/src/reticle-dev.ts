...
if (import.meta.env.DEV) {
  ...
  registerStore('editor', useEditorStore);
  registerStore('project', useProjectStore);
  registerStore('ui', useUIStore);

  registerCapabilities({
    testids: [], // none found; add data-testid to your key elements
    signals: [], // names you pass to reticle.signal()
    stores: ['auth', 'editor', 'project', 'ui'], // the keys you registered above
  });
}

...
async function openFileTreeModal(project) {
  ...
  try {
    ...
    const canCreate = true; // Anyone with project access can create files

    const body = document.createElement('div');

    const addBtn = canCreate ? `
      <div style="display: flex; gap: 8px;">
        <button class="btn btn-primary btn-sm" id="tree-add-room">+ New Room</button>
        <button class="btn btn-ghost btn-sm" id="tree-add-file" title="Advanced Options">+ File/Dir</button>
      </div>` : '';
    body.innerHTML = `
      <div class="filetree">
        <div class="filetree-header" style="display: flex; justify-content: space-between; align-items: center;">
          <span>Explorer</span>
          ${addBtn}
        </div>
    ...
    if (canCreate) {
      ...
      body.querySelector('#tree-add-room')?.addEventListener('click', () => openCreateRoomModal(project.id));
    }
...
function renderTreeHTML(nodes, depth, canDelete, projectId) {
  ...
  return nodes.map(n => {
    ...
    // For files, render a clickable link to the Notepad
    const nameEl = isFile 
      ? `<a href="/notepad/?projectId=${projectId}&fileId=${n.id}&token=${state.token}" style="flex:1;text-decoration:none;color:var(--text-primary);cursor:pointer;" title="Open in Notepad">${escHtml(n.name)}</a>`
...
function openCreateNodeModal(projectId, parentId) {
  ...
  $('m-create-node').addEventListener('click', async () => {
    ...
    try {
      const node = await api('POST', `/projects/${projectId}/tree`, { name, type, parent_id: parentId || undefined });
      toast('Created!', 'success');
      closeModal();
      // Auto-open files in Notepad in the same tab to avoid popup blockers
      if (type === 'file') {
        window.location.href = `/notepad/?projectId=${projectId}&fileId=${node.id}&token=${state.token}`;
      }
    } catch (err) {
      toast(err.message, 'error');
    }
...
function openCreateRoomModal(projectId) {
  openModal('Create Permanent Sharing Room',
    `<div class="form-body">
      <div class="form-field">
        <label>Room Name</label>
        <input id="m-room-name" type="text" placeholder="My Secure Room" />
      </div>
    </div>`,
    `<button class="btn btn-ghost" id="m-cancel-room">Cancel</button>
     <button class="btn btn-primary" id="m-create-room">Create Room</button>`
  );
  $('m-cancel-room').addEventListener('click', closeModal);
  $('m-create-room').addEventListener('click', async () => {
    const name = $('m-room-name').value.trim();
    if (!name) { toast('Room name is required', 'error'); return; }
    try {
      // 1. Generate the permanent key locally
      const secureKey = Array.from(crypto.getRandomValues(new Uint8Array(8)))
          .map(b => b.toString(16).padStart(2, '0')).join('') + '-' + Math.random().toString(36).slice(2, 6);
      
      // 2. Create the room (file) on the backend
      await api('POST', `/projects/${projectId}/tree`, { name, type: 'file' });
      
      // 3. Show the key to the admin
      openModal('Room Created!',
        `<div class="form-body text-center">
          <p>The sharing room <strong>${escHtml(name)}</strong> is ready.</p>
          <div style="background:var(--bg-canvas);padding:16px;margin:16px 0;border-radius:6px;border:1px dashed var(--border);">
            <p style="margin-bottom:8px;font-size:12px;color:var(--text-secondary)">Permanent Room Key</p>
            <span style="font-family:monospace;font-size:18px;color:var(--accent-blue);">${escHtml(secureKey)}</span>
          </div>
          <p class="text-danger" style="font-size:12px;"><strong>CRITICAL:</strong> Copy this key immediately! Because of Zero-Knowledge encryption, the server does not store it. If you lose it, the room's shared data is lost.</p>
        </div>`,
        `<button class="btn btn-primary btn-full" id="m-ack-room">I've copied the key</button>`
      );
      
      $('m-ack-room').addEventListener('click', () => {
        closeModal();
      });
